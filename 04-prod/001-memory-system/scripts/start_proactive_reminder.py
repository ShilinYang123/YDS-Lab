...
try:
    ...
    try:
        from .file_lock import FileLock  # 兼容包相对导入
    except Exception:
        FileLock = None
        ...
        try:
            ...
            if FileLock:
                with FileLock(self.memory_path):
                    ...
                    try:
                        importance = 0.6  # 默认重要性，后续可按类型/上下文动态计算
                        content = f"[Reminder][{reminder.get('type','general')}] {reminder.get('title','')} - {reminder.get('message','')}"
                        summary = reminder.get('message', '')
                        memory_entry = {
                            "id": f"mem_{int(datetime.now().timestamp()*1000)}",
                            "content": content,
                            "summary": summary,
                            "type": "semantic",  # 提醒多为最佳实践/建议，记为语义记忆
                            "importance": importance,
                            "tags": ["proactive_reminder", reminder.get('type', 'general')],
                            "context": reminder.get('context', {}),
                            "metadata": {
                                "source": "ProactiveReminder",
                                "reminderId": reminder.get('id'),
                                "acknowledged": reminder.get('acknowledged', False)
                            },
                            "createdAt": reminder.get('timestamp', datetime.now().isoformat()),
                            "updatedAt": reminder.get('timestamp', datetime.now().isoformat())
                        }
                        # 确保 memories 数组存在
