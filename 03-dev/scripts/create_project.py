...
class YDSLabProjectCreator:
    ...
    def __init__(self, project_root: str = "s:/YDS-Lab"):
        ...
        # 新优先路径：03-dev；兼容旧路径：projects（仅用于列出与读取，不作为创建目标）
        self.projects_dir = self.project_root / "03-dev"
    ...
    def setup_logging(self):
        ...
        try:
            ...
            self.logger.info("项目创建器初始化（日志：01-struc/logs）")
    ...
    def list_projects(self) -> List[str]:
        """列出已创建的项目"""
        # 仅列出 03-dev 下的项目（不再回退到 projects）
        search_dir = self.projects_dir
        if not search_dir.exists():
            self.logger.error("开发目录 03-dev 不存在。请创建 03-dev 并迁移项目后再执行。")
            return []
