...
优先使用 -sub/git_tools/git_helper.py 中的 GitHelper；若不存在，则尝试
使用 tools/git/auto_push.py 中的 YDSLabAutoPush 并封装为同名接口；再不行，
使用系统 git 命令作为最终回退以保证 fi.py 的推送流程正常。
"""

# 修复导入路径，使用相对导入，并提供健壮的回退实现
try:
    from pathlib import Path
    ...
    if git_helper_path.exists():
        ...
        if auto_push_path.exists():
            ...
            class GitHelper:
                """封装 YDSLabAutoPush 为 fi.py 期望的接口"""
                def __init__(self, repo_path: str):
                    self.repo_path = str(repo_path)
                    self.auto = YDSLabAutoPush(project_root=self.repo_path)
                    # 确保使用项目根目录的.gitignore文件
                    self._ensure_gitignore_exists()

                def _ensure_gitignore_exists(self):
                    """确保项目根目录存在.gitignore文件"""
                    gitignore_path = Path(self.repo_path) / ".gitignore"
                    if not gitignore_path.exists():
                        # 如果不存在，记录警告但不创建，避免干扰现有工作流
                        self.auto.logger.warning("项目根目录未找到.gitignore文件，某些文件可能被意外提交")
                    else:
                        self.auto.logger.info(f"使用项目根目录的.gitignore文件: {gitignore_path}")
                ...
                def get_commits(self, since: str = None, until: str = None):
                    # 使用底层 run_git_command 以支持时间范围过滤
                    ...
                    if ok and out.strip():
                        ...
                        return {'success': False, 'error': err or 'git log 执行失败', 'commits': []}
        else:
            # 最终回退：直接使用系统 git 命令实现必要接口
            class GitHelper:
                def __init__(self, repo_path: str):
                    self.repo_path = str(repo_path)
...
class YDSLabFinishProcessor:
    ...
    def __init__(self, project_root: str = "s:/YDS-Lab"):
        ...
        # 按照V5.1架构和三级存储规范更新目录路径
        self.docs_dir = self.project_root / "01-struc" / "docs"  # 公司级文档存储
        self.ai_dir = self.project_root / "01-struc" / "Agents"  # 修正AI目录路径，指向实际的Agents目录
        self.logs_dir = self.project_root / "logs"  # 系统维护类日志存储 - 按照三级存储规范
        self.bak_dir = self.project_root / "01-struc" / "bak"    # 全公司统一备份存储
        ...
        self.default_config = {
            'backup': {
                ...
                'exclude_patterns': [
                    ...
                    'Backups', 'bak', '*.bak', '*.backup',  # 备份文件
                    '*.log', '*.tmp', '*.temp', 'temp', 'cache', '.cache',  # 临时文件
                    'api_keys.json', '*.key', '*.pem', '.env', '.env.*',  # 敏感文件
                    'logs', '*.out', 'downloads', '*.zip', '*.tar.gz',  # 日志和大文件
    ...
    def check_ai_agents_status(self) -> Dict[str, any]:
        ...
        # 修正目录结构，适配实际的Agents目录结构
        agents_dir = self.ai_dir  # 直接指向01-struc/Agents目录
        ...
        try:
            # 检查Agent文件 - 适配实际的Agents目录结构
            if agents_dir.exists():
                # 只查找所有带编号的Agent目录（01-20）
                agent_dirs = [d for d in agents_dir.iterdir() if d.is_dir() and d.name.split('-')[0].isdigit()]
                status['total_agents'] = len(agent_dirs)
                status['agents_active'] = status['total_agents'] > 0
    ...
    def perform_git_push(self) -> Dict[str, any]:
        ...
        try:
            ...
            if not gitignore_path.exists():
                self.logger.warning("项目根目录未找到.gitignore文件，某些文件可能被意外提交")
            else:
                self.logger.info(f"使用项目根目录的.gitignore文件: {gitignore_path}")
