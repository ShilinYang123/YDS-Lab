...
class RBACSystem:
    ...
    def create_user(self, user_id: str, username: str, display_name: str, email: str, roles: Set[str]) -> bool:
        ...
        self._audit_log("system", "CREATE_USER", ResourceType.SYSTEM, user_id, True, 
                       details={"username": username, "roles": list(roles)})
    ...
    def has_permission(self, user_id: str, permission: Permission, resource_type: ResourceType = None, resource_id: str = None) -> bool:
        """检查用户是否有指定权限"""
        ...
        for acl in self.acl_entries:
            if (acl.user_id == user_id and 
                permission in acl.permissions and
                (not resource_type or acl.resource_type == resource_type) and
                (not resource_id or acl.resource_id == resource_id) and
                (not acl.expires_at or acl.expires_at > datetime.now())):
                return True
        
        return False
    
    def grant_permission(self, user_id: str, permission: Permission, resource_type: ResourceType, 
                        resource_id: str, granted_by: str, expires_at: datetime = None) -> bool:
        """授予权限"""
        ...
        self._audit_log(granted_by, "GRANT_PERMISSION", resource_type, resource_id, True,
                       details={"target_user": user_id, "permission": permission.value})
        
        return True
    
    def revoke_permission(self, user_id: str, permission: Permission, resource_type: ResourceType, 
                         resource_id: str, revoked_by: str) -> bool:
        """撤销权限"""
        removed = False
        self.acl_entries = [
            acl for acl in self.acl_entries 
            if not (acl.user_id == user_id and 
                   permission in acl.permissions and
                   acl.resource_type == resource_type and
                   acl.resource_id == resource_id)
        ]
        
        if removed:
            self._audit_log(revoked_by, "REVOKE_PERMISSION", resource_type, resource_id, True,
                           details={"target_user": user_id, "permission": permission.value})
    ...
    def _audit_log(self, user_id: str, action: str, resource_type: ResourceType, resource_id: str, 
                   success: bool, ip_address: str = None, user_agent: str = None, details: Dict[str, Any] = None):
        """记录审计日志"""
    ...
    def get_audit_logs(self, user_id: str = None, action: str = None, resource_type: ResourceType = None,
                      start_time: datetime = None, end_time: datetime = None, limit: int = 1000) -> List[AuditEntry]:
        """获取审计日志"""
        ...
        def decorator(func):
            ...
            def wrapper(*args, **kwargs):
                ...
                if not self.has_permission(user_id, permission, resource_type, resource_id):
                    self._audit_log(user_id, func.__name__, resource_type or ResourceType.SYSTEM, 
                                   resource_id or "unknown", False, 
