...
- 同域托管前端静态页面（仅 03-dev/JS001-meetingroom/ui）
- 实时日志传输到浏览器（WebSocket）

启动：
  python 03-dev/JS001-meetingroom/servers/meetingroom_server.py --host 127.0.0.1 --port 8020
...
# 导入新的系统组件
from mcp_message_model import MCPMessage, MCPMessageBuilder, MCPMessageValidator, ChannelType, EventType
from agent_roles import AgentRoleManager, AgentRole, MeetingLevel
from meeting_levels import MeetingLevelManager, MeetingStatus, VoteType
from intelligent_agenda import IntelligentAgendaGenerator, AgendaPriority, AgendaType
from document_governance import DocumentGovernanceManager, DocumentCategory, AccessLevel
from rbac_system import RBACSystem, Permission, ResourceType
from voice_service import VoiceServiceManager, VoiceServiceType, AudioFormat


# 仓库根目录（注意：本文件位于 03-dev/JS001-meetingroom/servers 下，根目录为 parents[3]）
...
# 前端目录：仅 03-dev/JS001-meetingroom/ui（不再兼容 projects 或其他旧路径）
UI_DIR_PRIMARY_03DEV = REPO_ROOT / "03-dev" / "JS001-meetingroom" / "ui"
...
LM_FALLBACK_PATCH_DIR = REPO_ROOT / "01-struc" / "05-marketing" / "Task" / "005-Trae长记忆功能实施" / "智能监控系统补丁包v1.0" / "src" / "tools" / "LongMemory"
...
# 统一‘项目目录’显示：仅 03-dev
PROJECT_DIR_JS001 = REPO_ROOT / "03-dev" / "JS001-meetingroom"
PROJECT_DIR_JS001_DISPLAY = "03-dev/JS001-meetingroom"

# 调试输出：在启动时打印与前端目录相关的信息，便于定位路径问题
try:
    sys.stdout.write("[UI] REPO_ROOT: " + str(REPO_ROOT) + "\n")
    sys.stdout.write("[UI] UI_DIR_PRIMARY_03DEV: " + str(UI_DIR_PRIMARY_03DEV) + " (exists: " + str(UI_DIR_PRIMARY_03DEV.exists()) + ")\n")
    sys.stdout.write("[UI] PROJECT_DIR_JS001_DISPLAY: " + str(PROJECT_DIR_JS001_DISPLAY) + "\n")
    idx_03 = UI_DIR_PRIMARY_03DEV / "index.html"
    sys.stdout.write("[UI] Selected UI_DIR: " + str(UI_DIR) + " (exists: " + str(UI_DIR.exists()) + ")\n")
...
WS_JWT_REQUIRED: bool = str(os.environ.get("WS_JWT_REQUIRED", "false")).strip().lower() in ("1", "true", "yes", "on")

# 统一的 MCP 消息模型封装（MVP）
def emit_mcp_event(event: str, data: Dict[str, Any], level: Optional[str] = None, channel: Optional[str] = None, broadcast: bool = True) -> None:
...
try:
    ...
    LM_CONFIG_FALLBACK_PATCH = REPO_ROOT / "01-struc" / "05-marketing" / "Task" / "005-Trae长记忆功能实施" / "智能监控系统补丁包v1.0" / "config" / "LongMemory"
...
def api_book_room():
    ...
    try:
        ...
        # 检查时间冲突
        for booking in BOOKINGS_STORE:
            if (booking["roomId"] == data["roomId"] and 
...
def mcp_docs_revoke():
    ...
    # 资源操作权限
    cfg = _load_permission_cfg()
    revoke_roles = (((cfg.get("resource_permissions") or {}).get("meetingroom") or {}).get("docs") or {}).get("operations", {}).get("revoke", {}).get("allowed_roles", [])
...
def mcp_lm_report_error():
    ...
    try:
        ...
        emit_mcp_event("lm_error", {"type": error_type, "message": error_message, "context": context, "by": role}, level="warning")
...
def _votes_allowed_roles(op: str) -> List[str]:
    cfg = _load_permission_cfg()
    return (((cfg.get("resource_permissions") or {}).get("meetingroom") or {}).get("votes") or {}).get("operations", {}).get(op, {}).get("allowed_roles", [])

def _voice_allowed_roles(op: str) -> List[str]:
    cfg = _load_permission_cfg()
    return (((cfg.get("resource_permissions") or {}).get("meetingroom") or {}).get("voice") or {}).get("operations", {}).get(op, {}).get("allowed_roles", [])
...
def mcp_votes_cast():
    ...
    if not _is_role_allowed_for_endpoint("/mcp/votes/*", role):
        _audit_log("vote_cast_denied", {"role": role, "vote_id": vid, "option": option, "reason": "endpoint_role_denied"})
        return jsonify({"ok": False, "error": "endpoint_role_denied"}), 403
    if role not in _votes_allowed_roles("cast"):
        _audit_log("vote_cast_denied", {"role": role, "vote_id": vid, "option": option, "reason": "role_not_allowed"})
        return jsonify({"ok": False, "error": "role_not_allowed"}), 403
    ...
    try:
        socketio.emit("vote_cast", {"vote_id": vid, "option": option, "voter": voter, "counts": counts}, broadcast=True)
    except Exception:
        pass
    try:
        emit_mcp_event("vote_cast", {"vote_id": vid, "option": option, "voter": voter, "counts": counts})
    except Exception:
        pass
...
def mcp_voice_chunk():
    ...
    _audit_log("vote_finalize", {"role": role, "vote_id": vid, "status": "granted"})
    return jsonify({"ok": True, "vote": rec})
...
def mcp_voice_stream():
    ...
    try:
        socketio.emit("voice_stream", {"meta": meta}, broadcast=True)
    except Exception:
        pass
    try:
        emit_mcp_event("voice_stream", {"meta": meta})
    except Exception:
        pass
        ...
        for role, config in roles.items():
            ...
            try:
                serializable_roles[role.value] = {
                    ...
                    "created_at": getattr(config, 'created_at', None).isoformat() if getattr(config, 'created_at', None) else None,
