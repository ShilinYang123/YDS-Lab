...
class MCPIntegrationValidator:
    ...
    def __init__(self, project_root: str = None):
        ...
        # 统一解析 MCP 集群目录：
        # 1) 新路径优先：tools/mcp/servers
        # 2) 兼容回退：01-struc/0B-general-manager/tools/mcp/servers（若历史结构存在）
        # 3) 最后回退：Struc/MCPCluster（历史文档中提及的旧位置）
        candidates = [
            self.project_root / "tools" / "mcp" / "servers",
            self.project_root / "01-struc" / "0B-general-manager" / "tools" / "mcp" / "servers",
            self.project_root / "Struc" / "MCPCluster",
        ]
    ...
    def _load_cluster_config(self):
        """加载MCP集群配置：新路径优先，兼容旧路径"""
        cfg_candidates = [
            self.mcp_cluster / "cluster_config.yaml",
            self.project_root / "config" / "mcp_cluster_config.yaml",  # 未来若迁移至 /config，可在此读取
        ]

        for config_file in cfg_candidates:
            try:
        ...
        print("❌ 未找到有效的 MCP 集群配置文件 (尝试: tools/mcp/servers/cluster_config.yaml 与 config/mcp_cluster_config.yaml)")
        return None
    ...
    def _generate_validation_report(self):
        ...
        report_file = report_dir / "mcp_validation_report.json"
