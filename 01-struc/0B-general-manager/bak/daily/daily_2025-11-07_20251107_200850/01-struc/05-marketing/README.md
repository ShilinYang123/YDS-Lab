...
组织协作接口
- 关联可执行角色：`01-struc/Agents/marketing_director/`（市场总监智能体）；协同 `01-struc/Agents/resource_admin/` 进行文档与资源治理；与 `01-struc/Agents/dev_team/` 对接项目实施。
...
运行参与说明
- 本目录通常不包含可执行代码；执行由相关 Agents 完成（市场总监、开发团队与资源管理员协作）。
...
快速索引
- 市场总监智能体：`01-struc/Agents/marketing_director/`
