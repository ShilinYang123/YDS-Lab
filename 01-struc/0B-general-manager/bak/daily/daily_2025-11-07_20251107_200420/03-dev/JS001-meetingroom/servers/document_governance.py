...
class DocumentGovernanceManager:
    ...
    def __init__(self, config_path: str = None):
        # 统一配置路径优先使用 config/document_governance_config.json，保留根路径回退
        if config_path:
        ...
        if not self.directory_rules:
            default_rules = [
                DirectoryRule(
                    ...
                    description="YDS AI战略规划文档",
                    auto_approval=False,
                    requires_audit=True
                ),
                DirectoryRule(
                    path="S:/YDS-Lab/01-struc/05-marketing",
                ...
                DirectoryRule(
                    ...
                    allowed_roles={"CTO", "前端开发", "后端开发", "项目协调者"},
                    access_levels={"CTO": AccessLevel.ADMIN, "前端开发": AccessLevel.WRITE, "后端开发": AccessLevel.WRITE, "项目协调者": AccessLevel.READ},
                    description="技术部门文档",
                    auto_approval=True,
                    requires_audit=True
                ),
                ...
                DirectoryRule(
                    ...
                    allowed_roles={"CEO", "项目协调者", "前端开发", "后端开发"},
                    access_levels={"CEO": AccessLevel.ADMIN, "项目协调者": AccessLevel.WRITE, "前端开发": AccessLevel.WRITE, "后端开发": AccessLevel.WRITE},
                    description="生产环境文档",
                    auto_approval=True,
                    requires_audit=True
                ),
                DirectoryRule(
                    ...
                    category=DocumentCategory.TECHNICAL,
                    allowed_roles={"CTO", "前端开发智能体", "后端开发智能体", "项目协调者"},
                    access_levels={"CTO": AccessLevel.ADMIN, "前端开发智能体": AccessLevel.WRITE, "后端开发智能体": AccessLevel.WRITE, "项目协调者": AccessLevel.READ},
                    description="工具和脚本",
                    auto_approval=True,
    ...
    def check_access(self, user_role: str, path: str, action: AuditAction, user_id: Optional[str] = None) -> Tuple[bool, str]:
        """检查访问权限"""
        # 标准化路径
        ...
        # 检查角色权限
        if "*" not in matching_rule.allowed_roles and user_role not in matching_rule.allowed_roles:
            return False, f"角色 {user_role} 无权访问此目录"
        
        # 检查访问级别
        required_level = self._get_required_access_level(action)
        user_level = matching_rule.access_levels.get(user_role) or matching_rule.access_levels.get("*", AccessLevel.NONE)
        
        if not self._has_sufficient_access(user_level, required_level):
            return False, f"权限不足，需要 {required_level.value} 权限"
    ...
    def request_access(self, user_id: str, user_role: str, path: str, action: AuditAction, reason: str = None) -> str:
        ...
        if matching_rule and matching_rule.auto_approval:
            allowed, message = self.check_access(user_role, path, action)
            if allowed:
    ...
    def log_access(self, user_id: str, user_role: str, path: str, action: AuditAction, success: bool, details: Dict[str, Any] = None):
        """记录访问日志"""
        log_id = hashlib.md5(f"{user_id}_{path}_{action.value}_{datetime.now().isoformat()}".encode()).hexdigest()
    ...
    def generate_access_report(self) -> Dict[str, Any]:
        ...
        for rule in self.directory_rules.values():
            category = rule.category.value
            if category not in report["按分类统计"]:
                report["按分类统计"][category] = 0
            report["按分类统计"][category] += 1
