...
> 仓库集成补充说明（YDS-Lab 使用）
>
> 为便于在本仓库中直接使用长记忆功能，已提供 Python 版 LongMemory 组件与 meetingroom_server 接口：
>
> - 统一持久化文件：`logs/longmemory/lm_records.json`
 > - 可配置覆盖：环境变量 `YDS_LONGMEMORY_STORAGE_PATH` 或 `config/yds_ai_config.yaml` 的 `longmemory.storage_path`（旧路径 `tools/servers/yds_ai_config.yaml` 已废弃，且不再提供回退读取）
> - 核心接口：
>   - 启动组件：POST `/mcp/lm/start`（`{"role":"developer","component":"monitor|reminder"}`）
>   - 停止组件：POST `/mcp/lm/stop`
>   - 状态查询：GET `/mcp/lm/status?role=developer`
>   - 活动上报：POST `/mcp/lm/report_activity`
>   - 错误上报：POST `/mcp/lm/report_error`
>
> 说明：旧的 `docs/02-开发/memory.json` 路径已废弃，现与 Trae IDE Memory MCP 完全解耦。首次启动后产生的“频繁修改”提醒属于预期行为；如控制台中文字出现乱码，可设置 PowerShell 输出编码为 UTF-8。
