...
def archive_meeting(content: str) -> str:
    ...
    统一归档目录到 01-struc/0B-general-manager/meetings；若目录不存在则自动创建。
    """
    base_ts = datetime.now().strftime('%Y%m%d-%H%M')
    # 解析仓库根目录（本文件位于 01-struc/agents/01-ceo/tools.py）
    repo_root = Path(__file__).resolve().parents[3]
    base_dir = repo_root / "01-struc" / "0B-general-manager" / "meetings"
    os.makedirs(str(base_dir), exist_ok=True)

    base_name = f"MTG-{base_ts}"
    md_path = os.path.join(str(base_dir), f"{base_name}.md")
    ...
    json_path = os.path.join(str(base_dir), f"{base_name}.json")
    try:
