...
- 各类日志 logs：遵循上述分层位置（公司级 → `01-struc\docs\...\logs`；部门级 → `01-struc\部门名\logs`；项目级 → `项目名\logs`；顶层运维 → `docs\logs`）。
...
- 日志 logs：与文档同路径层级维护，统一采用 `.log` 或日期前缀文件名；建议开启日志轮转（tools\LongMemory\log_rotate.ps1 可参考）。
...
- 例外清单（以下一级子目录名称作为保留位，部分可不使用编号前缀）：
  - `docs/`、`Docs/`、`logs/`、`Log/`、`config/`、`tools/`、`meetings/`、`archive/`、`archives/`、`backups/`。
- 示例：
