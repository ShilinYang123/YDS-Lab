...
四、详细任务拆解
1) 持久化目录与初始文件
   - 当前统一路径：01-struc/0B-general-manager/logs/longmemory/lm_records.json
   - 初始化内容（首次运行时自动或人工）：{"general": {}, "memories": []}
   - 说明：战略初稿中曾提出“01-struc/Log/longmemory”路径作为优化方向；现阶段沿用 0B-general-manager/logs/longmemory，待目录治理完成后再迁移并提供兼容映射。
