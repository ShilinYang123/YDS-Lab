...
class MonitorTester:
    ...
    def __init__(self):
        self.test_dir = Path("s:/3AI/test_workspace")
...
'''
    ...
    def test_memory_integration(self):
        ...
        try:
            ...
            # 查找智能监控相关的记录
            monitor_entries = [
                entry for entry in memory_data.get('entries', [])
                if 'monitor' in entry.get('content', {}).get('type', '').lower() or
                   'error' in entry.get('content', {}).get('type', '').lower()
            ]
