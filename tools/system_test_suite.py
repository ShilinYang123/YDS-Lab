...
class TraeSystemTestSuite:
    ...
    def _resolve_role_dir(self, *candidates: str) -> Path:
        """在 Agents 目录中解析角色目录，兼容大小写、数字前缀与不同命名。

        支持以下匹配策略：
        - 直接命中候选名称（大小写不敏感）
        - 目录名以 "NN-" 数字前缀开头，后缀部分与候选匹配（如 01-ceo）
        - 目录名包含候选作为后缀（如 03-planning_director）
        """
        # 直接匹配（大小写不敏感）
        ...
        if self.trae_agents_path.exists():
            for child in sorted(self.trae_agents_path.iterdir(), key=lambda x: x.name.lower()):
                if child.is_dir():
                    name_lower = child.name.lower()
                    # 移除可能的数字前缀
                    parts = name_lower.split('-', 1)
                    suffix = parts[-1] if len(parts) > 1 else name_lower
                    if any(suffix == c or name_lower.endswith(c) for c in cands_lower):
                        return child
    ...
    def test_fault_tolerance(self):
        ...
        import datetime
        backup_dir = self.project_root / "bak" / "test_backups"
        backup_dir.mkdir(parents=True, exist_ok=True)
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        backup_file = backup_dir / f"production.yaml.backup.{timestamp}"
