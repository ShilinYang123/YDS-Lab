...
class ProductionDeploymentManager:
    ...
    def _backup_existing_system(self):
        ...
        # 迁移说明：目录已合并为 0B-general-manager/bak，旧路径 GeneralOffice/bak 仅作为历史记录，不再回退使用
        backup_dir = self.project_root / "01-struc" / "0B-general-manager" / "bak" / f"v1_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
