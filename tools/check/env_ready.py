...
'01-struc/0B-general-manager/logs',
    ...
    def setup_logging(self):
        """设置日志系统"""
        try:
            logs_dir = self.project_root / "logs"  # 按照三级存储规范：系统维护类日志存储在logs/
    ...
    def check_directory_structure(self) -> Dict:
        ...
        for dir_name in self.required_dirs:
            ...
            if dir_name == '03-dev':
                # 零容忍：必须存在 03-dev；不再回退到 projects
        ...
        if missing_dirs:
            result['status'] = 'fail'
            result['messages'].append(f"缺少必需目录: {missing_dirs}（已禁用对旧目录的兼容回退）")
        else:
    ...
    def check_config_files(self) -> Dict:
        ...
        config_files = [
            ...
            # 兼容入口已废弃：tools/finish.py → 使用根目录 fi.py
            ('fi.py', 'required'),
('01-struc/0B-general-manager/config/project_config.yaml', 'optional'),
            ('env/requirements.txt', 'optional'),
            ('bak/backup_config.yaml', 'optional')
        ]
    ...
    def check_write_permissions(self) -> Dict:
        ...
        test_dirs = [
            ...
            # 新优先目录
            self.project_root / '03-dev',
            # 日志统一：总经办日志路径迁移至 01-struc/0B-general-manager/logs
            self.project_root / '01-struc' / '0B-general-manager' / 'logs'
        ]
